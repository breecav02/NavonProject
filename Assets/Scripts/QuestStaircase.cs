...
public class QuestStaircase :MonoBehaviour
{
    ...
    /// This script was inherited from Sujin, who found it online. But it implements a QUEST staircase based on the parameters defined in questparam.
    /// </summary>

    // MD note that I'm using Mathf, so need to explicitly convert all the old Math functions ,to Mathf using (float)


    // preallocate parameters:
    //#pragma warning disable IDE0044 
    float tGuess,  tGuessSd, pThreshold,  xThreshold, beta, delta, gamma, grain, range, plotIt, dim;
    int nConds = 2; // two walking speeds

    //first distributions
    public int[] i;
    public float[] x, pdf;
    //second distribution
    public float[] i2, x2, p2;
    // 
    bool updatePdf, warnPdf, normalizePdf;
    ...
    public QuestStaircase[] questStair; // using constructor to create multiple instances of the staircase.
    ...
    private void Start()
    {
        ...
        tGuess = 1f; //start easy
        tGuessSd = 2f;
        pThreshold = 0.75f; //was .55f
        beta = 3.5f; // steepness of psychometric fxn
        delta = 0.01f; // lapse rate (fraction of trials ppant responses are random)
        gamma = .01f; //(.5f) is the fraction of trials that will generate response 1 when intensity == -in
        grain = .15f;////  .02f quantization(step size) of the interval table
        range =2f; //// .6f in same scale. range/2:tGuess:range/2
        //have restricted range to avoid negative values.

        nConds = 2;
        // Initialize the array to hold two QuestStaircase instances
        questStair = new QuestStaircase[nConds+1]; //populated in InitQuestParams() +1 extra for the stationary conds.
        

    }

    // constructor for QuestStaircase, called for each instance of the staircase.   
    ...
    public void InitQuestParams()
    {
        // create n=nConds quest staircases for the current session.
        // One per condition, retrieved when we are in that condition type

            
        // Create two QuestStaircase instances with the specified parameters
        for (int stair = 0; stair < nConds+1; stair++)
        {
            questStair[stair] = new QuestStaircase(
            tGuess,
            tGuessSd,
            pThreshold,
            beta,
        ...
        Debug.Log("Quest Staircases initialized for " + nConds + " conditions.");
        
    }


    public void UpdateStaircase(int stair_id, float intensity, int response)
    {
        // update specific staircase with the response [0 1];
        questStair[stair_id].UpdateQ(intensity, response);


        // return the new threshold estimate:
        questStair[stair_id].Mean();

        Debug.Log("new threshold estimate for staircase [" + stair_id + "] is: " + questStair[stair_id].tGuess);

        
        // pass new estimate to make revised gabor stimulus.
    }
